...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    ...
    def tokenize_text(text):
        ...
        if data_cfg["sign_type"] == 'CSL':
            captions = []
            for word in list(jieba.cut(text, cut_all=False)):
    ...
    def tokenize_features(features):
        ...
        if data_cfg["feature_type"]=='kmeans': 
            #new changes for kmeans
            features_new = np.asarray(features)
            ft_list_new = torch.FloatTensor(features_new + 2)
            
            ft_list = torch.split(ft_list_new, 1, dim=0)
            k_means_return_val=[i for i in ft_list]
            return k_means_return_val
        else:
