...
class MULTFusion(nn.Module):
    def __init__(self, conf):
        ...
        self.enc_hidden_d = self.enc_emb
        ...
        self.tri_fusion_att = self.get_network( (self.enc_emb*3), self.enc_attn_dropout_mem )
    ...
    def forward(self, f1, f2, f3, txt_input, txt, f1_mask, txt_mask, sgn_lengths, capture_wts=False ):
        ...
        f2 = F.dropout(f2.transpose(1, 2), p=self.enc_embed_dropout, training=self.training)
        f3 = F.dropout(f3.transpose(1, 2), p=self.enc_embed_dropout, training=self.training)
        # f2 = f2.transpose(1, 2)
        # f3 = f3.transpose(1, 2)
        ...
        if self.f2_only:   
            ...
            h_f2 = self.trans_f2_mem( proj_f2 )
        ...
        if self.f3_only:
            ...
            h_f3 = self.trans_f3_mem(proj_f3)
        ...
        if self.channels == 3:
            ...
            encoder_outputs = self.tri_fusion_att(encoder_outputs) #trimodal
